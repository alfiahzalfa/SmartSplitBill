...
def model_selection_view(settings: SettingsData) -> SettingsData:
    ...
    if selected == ModelNames.GEMINI:
        ...
        st.warning("⚠️ **Dijalankan pada perangkat lokal**")
...
def controller(error_msg: str | None = None) -> None:
    ...
    st.markdown("Sesuaikan pengaturan sesuai preferensimu. Perubahan akan langsung diterapkan.")
    st.markdown("---")
    ...
    st.markdown("---")
    if st.button("Save Settings", key="settings_apply_button", type="primary", use_container_width=True):
        settings.apply()
        st.rerun()